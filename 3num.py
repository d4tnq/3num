...
from lib.colors import debug, info, error,Fore
...
class ActiveEnum:
    def __init__(self, verbose=0, deepscan=False, outdir='results', srvname='',username='',password=''):
    ...
    def run_nmap(self, address):
        ...
        self.run_cmd(f'nmap -sV -sC -p0-10000 -T4 -oX "{nmap_file}" {address}')
    ...
    def enum_service(self, address, port, service):
        info(f'Enumerating '+Fore.LIGHTMAGENTA_EX+'{service}'+Fore.RESET+' on {address}:{port}')
        basedir = os.path.join(self.outdir, address + self.srvname)
        os.makedirs(basedir, exist_ok=True)

        if 'smb' in service or ('netbios' in service and port in [139, 445]):
            SMB_enum(address, port, service, basedir,self.verbose, self.username, self.password)
...
if __name__ == '__main__':
    print(Fore.LIGHTCYAN_EX+banner.intro)
    ...
    parser.add_argument('-u', '--username', type=str, help='Username to use (bruteforce, smb,...)')
    parser.add_argument('-k', '--password', type=str, help='Password to use (bruteforce, smb,...)')

    args = parser.parse_args()

    scanner = ActiveEnum(verbose=args.verbose,outdir=args.outdir,username=args.username,password=args.password)
    if args.port:
