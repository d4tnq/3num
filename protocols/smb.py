...
class SMB_enum (Protocol):
	def __init__(self, host, port, service, basedir,verbose, username, password):
		...
		self.run_cmds([
			...
			(
				e('enum4linux -u {username} -p {password} -a -M -l -d {host} | tee "{basedir}/{port}_smb_enum4linux.txt"'),
				e('enum4linux-{port}')
			),			
			(
